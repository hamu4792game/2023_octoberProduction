...
void MusicScore::SetNotes(ScoreType type, std::vector<Vector3> position, int32_t offset) {
	...
	case MusicScore::Easy_09:
		...
		break;
	case MusicScore::Easy_10:

		for (int i = 0; i < position.size() - 1; i++) {
			if (i == 0 || (i % 2 == 0 && i < 14 && i >= 8)) {
